...
const Checkout: React.FC = () => {
  ...
  useEffect(() => {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'reservas', filter: `id=in.(${trackedReservationIds.join(',')})` }, (payload) => {
            if (payload.new.payment_status === PaymentStatus.PAGO) setIsSuccess(true);
        })
        .subscribe();
    ...
    try {
        ...
        if (mode === 'CLIENT_ONLINE' && settings?.onlinePaymentEnabled) {
             ...
             if (checkoutUrl) {
                 ...
                 alert("Não foi possível gerar o pagamento online agora. Tente novamente ou pague no local.");
             }
        } else if (mode !== 'CLIENT_ONLINE' || !settings?.onlinePaymentEnabled) {
            setIsSuccess(true);
        }
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col">
       <header className="bg-slate-900 p-4 shadow-md border-b border-slate-800">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
           {!imgError ? (<img src="/logo.png" className="h-10 md:h-14 object-contain" onError={() => setImgError(true)}/>) : (<h1 className="text-2xl font-bold text-neon-orange uppercase">TÔ NA PISTA</h1>)}
          ...
          <div className="lg:col-span-1">
             <div className="bg-slate-900 rounded-xl p-6 border border-slate-800 shadow-lg">
                ...
                <div className="space-y-4 text-sm">
                    ...
                    <div className="flex justify-between"><span>Horário:</span> <span className="text-white">{reservationData.time}</span></div>
                    <div className="border-t border-slate-700 pt-4 flex justify-between items-center"><span className="font-bold text-lg">Total</span><span className="font-bold text-2xl text-neon-green">{reservationData.totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>
                </div>
             </div>
          </div>
            ...
            {currentUser ? (
                        ...
                        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-4">
                                {(['DINHEIRO', 'PIX', 'DEBITO', 'CREDITO'] as PaymentMethodStaff[]).map(m => (
                                    <button key={m} onClick={() => setStaffMethod(m)} className={`p-2 rounded text-xs font-bold border transition ${staffMethod === m ? 'bg-neon-green text-black border-neon-green' : 'bg-slate-700 border-slate-600'}`}>{m}</button>
                                ))}
                            </div>
                            <button onClick={() => processPayment('STAFF_CONFIRM')} disabled={isProcessing} className="w-full bg-green-600 text-white font-bold py-3 rounded-lg">{isProcessing ? <Loader2 className="animate-spin mx-auto"/> : 'Confirmar'}</button>
                        </div>
                        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                            <h3 className="text-sm font-bold text-yellow-500 mb-4 uppercase">Pagar no Local</h3>
                            <input type="text" placeholder="Comanda/Mesa" className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white mb-4" value={comandaInput} onChange={e => setComandaInput(e.target.value)} />
                            <button onClick={() => processPayment('STAFF_LATER')} disabled={isProcessing} className="w-full bg-slate-700 text-white font-bold py-3 rounded-lg">Salvar na Comanda</button>
                        </div>
                    </div>
                </div>
            ) : (
                <div className="bg-slate-900 rounded-xl p-8 border border-slate-800 shadow-lg text-center">
                    <div className="flex flex-col items-center gap-6">
                        ...
                        <button onClick={() => processPayment('CLIENT_ONLINE')} disabled={isProcessing} className="w-full max-w-sm bg-neon-blue hover:bg-blue-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2">
