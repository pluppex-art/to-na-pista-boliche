...
const Login: React.FC = () => {
  ...
  // NÃO faz mais logout automático ao abrir a tela de login.
  // Apenas carrega configurações visuais.
  useEffect(() => {
  ...
  const handleStaffLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se refreshUser for assíncrono, isso ainda funciona; se for síncrono, não quebra.
      try {
        await Promise.resolve(refreshUser());
      } catch (e) {
        console.error('Erro ao atualizar contexto de usuário:', e);
      }
      ...
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-2xl shadow-2xl z-10 overflow-hidden">
        <div className="text-center p-8 pb-4 flex flex-col items-center">
          {!imgError ? (
            ...
            <h1 className="text-2xl font-bold text-neon-orange font-sans tracking-tighter uppercase mb-2">
              {establishmentName}
            </h1>
          )}
        </div>
        ...
        <div className="flex border-b border-slate-700">
          <button 
            ...
            className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition ${
              activeTab === 'CLIENT' 
                ? 'text-white border-b-2 border-neon-blue bg-slate-800/50' 
          ...
          <button 
            ...
            className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition ${
              activeTab === 'STAFF' 
                ? 'text-white border-b-2 border-neon-orange bg-slate-800/50' 
                : 'text-slate-500 hover:text-white hover:bg-slate-800/30'
            }`}
          >
                ...
                <form onSubmit={handleClientLogin} className="space-y-5">
                  <div>
                    ...
                    <input
                      type="email"
                      required
                      className="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-neon-blue outline-none"
                      value={email}
                      onChange={e => setEmail(e.target.value)}
                    />
                  </div>
                ...
                <form onSubmit={handleClientRegister} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      className="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-2 text-white focus:border-neon-blue outline-none"
                      value={name}
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full bg-neon-green hover:bg-green-600 text-white font-bold py-3 rounded-lg shadow-lg transition flex items-center justify-center gap-2"
                  >
        ...
        <div className="bg-slate-950 p-4 text-center border-t border-slate-800">
          <Link
            to="/agendamento"
            className="text-sm text-slate-400 hover:text-white flex items-center justify-center gap-1 transition"
          >
